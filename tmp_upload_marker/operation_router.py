...
async def run_mapping_plotting(request: MappingPlottingRequest, user_info: dict = Depends(verify_token)):
    ...
    try:
        user_id = str(user_info['user_id'])
        api_root = os.path.abspath(os.path.join(R_CODE_DIRECTORY, ".."))
        r_script_path = os.path.join("string", "String_Workflow_Update_v3_Feb19.R")
        ...
        # Run the workflow from the API root so all relative paths resolve consistently.
        result = subprocess.run(
            command,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
